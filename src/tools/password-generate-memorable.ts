...
export function registerPasswordGenerateMemorable(server: McpServer): void {
  server.registerTool("password_generate_memorable", { description: "Generate a memorable passphrase from random dictionary words with optional number and symbol suffixes. Uses a ~500-word curated list for good entropy.", inputSchema: z.object({
              wordCount: z
                .number()
                .int()
                .min(2)
